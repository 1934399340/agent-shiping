...
function parseKeywordList(text) {
    ...
    const segments = keywords.map(keyword => ({
        text: keyword,
        keywords: [keyword],
        duration: 3, // 每个关键词3秒
        mood: 'neutral',
    ...
    for (const line of lines) {
        ...
        if (parts.length >= 1 && parts[0]) {
            segments.push({
                ...
                mood: parts[3] || detectMood(parts[0]),
            });
        }
    }
    return {
...
// 推荐音乐
function suggestMusic(segments) {
    // 统计情绪分布
    const moodCounts = {};
